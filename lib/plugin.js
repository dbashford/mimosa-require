"use strict";
var builder, fs, logger, optimizer, path, requireRegister, wrench, _, _buildDone, _buildOptimizeConfigs, _buildOptimizeConfigsFile, _clean, _removeCombined, _requireDelete, _requireOptimize, _requireRegister,
  __slice = [].slice;
...
_clean = function(config, options, next) {
  ...
  if (fs.existsSync(jsDir)) {
    files = wrench.readdirSyncRecursive(jsDir).filter(function(f) {
      return /-built\.js(\.map|\.src)?$/.test(f) || /almond\.js$/.test(f);
    }).map(function(f) {
      f = path.join(jsDir, f);
      fs.unlinkSync(f);
...
_requireRegister = function(config, options, next) {
  ...
  options.files.forEach(function(file) {
    if (file.outputFileName && file.outputFileText) {
      if (!file.outputFileName.match(/\.js$/)) {
        return;
      }
      if (config.isVirgin) {
        return requireRegister.process(file.inputFileName, file.outputFileText);
      } else {
        return requireRegister.process(file.outputFileName, file.outputFileText);
      }
    }
  });
  return next();
...
_requireOptimize = function(config, options, done) {
  ...
  next = function() {
    if (i < options.runConfigs.length) {
      return optimizer.execute(options.runConfigs[i++], next);
    } else {
      return done();
    }
  };
