...
module.exports = RequireRegister = (function() {
  ...
  RequireRegister.prototype.processed = {};

  RequireRegister.prototype.syncCall = 0;
  ...
  */
  ...
  RequireRegister.prototype._parse = function(fileName, source) {
    var err, isRequireFile, modName, numCommonJS, rci, result, withoutCommonJS;
    ...
    if (rci.requireCount) {
      ...
      if (numCommonJS !== rci.requireCount) {
        isRequireFile = true;
      }
    }
    return {
  ...
  RequireRegister.prototype._verifyAll = function() {
    var baseFile, deps, file, i, shims, total, v, _i, _len, _ref, _ref1, _ref2, _ref3, _ref4;
    ...
    this._buildTree();
    _ref3 = this.tree;
    for (baseFile in _ref3) {
      deps = _ref3[baseFile];
      console.log("\n" + deps.sort().join("\n"));
    }
    total = 0;
    _ref4 = this.processed;
    for (i in _ref4) {
      v = _ref4[i];
      total += v;
    }
    console.log("TOTAL IS ", total);
    return console.log("SYNC CALLS,", this.syncCall);
  };
  ...
  RequireRegister.prototype._addDepsToTree = function(f, dep, origDep) {
    ...
    if (this.processed[dep]) {
      this.processed[dep] = this.processed[dep] + 1;
    } else {
      this.processed[dep] = 1;
    }
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      aDep = _ref[_i];
      if (this._inAnyTree(aDep)) {
        if (this.tree[f].indexOf(aDep) === -1) {
          this.tree[f].push(aDep);
          if (this.depsRegistry[aDep]) {
            this.depsRegistry[aDep].forEach(function(depToAdd) {
              var aliasDep;
              aliasDep = _this._findAlias(depToAdd, _this.aliasFiles);
              depToAdd = aliasDep || depToAdd;
              if (!(_this.tree[f].indexOf(depToAdd) > -1)) {
                return _this.tree[f].push(depToAdd);
              }
            });
          }
        }
