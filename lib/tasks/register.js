...
module.exports = RequireRegister = (function() {
  ...
  */
  ...
  RequireRegister.prototype._verifyDep = function(fileName, dep) {
    var alias, cssPath, fileAliasList, fullDepPath, pathWithDirReplaced, plugin, _i, _len, _ref, _ref1;
    ...
    if (dep.indexOf('!') >= 0) {
      ...
      if (plugin === "css") {
        cssPath = path.join(this.rootJavaScriptDir, dep) + ".css";
        if (!fs.existsSync(cssPath)) {
          this._logger(("Path for CSS plugin [[ " + dep + " ]], inside file [[ " + fileName + " ]], cannot be found. Was resolved to [[ ") + cssPath + " ]]");
        }
        return;
      } else {
