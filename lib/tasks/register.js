...
module.exports = RequireRegister = (function() {
  ...
  RequireRegister.prototype.dependencyInfo = function() {
    var aliasResolved, config, dep, depPath, deps, f, mappedPath, name, reg, resolvedDirectory, resolvedPath, shimConfigs, shimOrigConfigFile, _i, _j, _len, _len1, _ref, _ref1;
    reg = {};
  ...
  */
  ...
  RequireRegister.prototype._mergeOriginalConfig = function(config) {
    var _this = this;
    return ["shim", "paths", "map", "config"].forEach(function(name) {
      var conf;
      if (config[name] != null) {
        conf = _.clone(config[name]);
        if (_this.originalConfig[name] != null) {
          return _this.originalConfig[name] = _.extend(_this.originalConfig[name], conf);
        } else {
          return _this.originalConfig[name] = conf;
        }
      }
    });
  };
  ...
  RequireRegister.prototype._verifyDep = function(fileName, dep) {
    var alias, fullDepPath, pathWithDirReplaced, plugin, _ref;
